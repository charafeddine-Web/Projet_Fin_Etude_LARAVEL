...
@section('title', 'Home  Module')
 
@section('contents')
<div class=" ">
    <h1 class="font-bold text-2xl ml-3">Home  Module</h1><hr />
    <div class="flex items-center justify-center bg-grey-lighter">
        

        <form action="{{ route('Searchmodule') }}" method="GET">
        ...
        <form action="{{ route('admin/modules/importExcel') }}" method="POST" enctype="multipart/form-data">
                @csrf
        ...
        <a href="{{ route('admin/modules/create') }}" class="rounded-lg ml-10 relative w-10 h-10 cursor-pointer flex items-center border  bg-green-300 group hover:bg-green-500 ">
    ...
    <table class="w-full overflow-x-auto shadow rounded-lg text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs min-w-full text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">Code Module</th>
                <th scope="col" class="px-6 py-3">Nom Module</th>
            ...
            <tr class=" border-b bg-[#f8f4f3]  dark:border-gray-700 hover:bg-gray-600 dark:hover:bg-gray-300">
                ...
                <td class="px-6 py-4 whitespace-nowrap w-full md:w-auto">
                    <div class="h-14 pt-5 flex justify-center md:justify-start">
                        <a href="{{ route('admin/modules/show', $rs->id) }}" class="text-blue-800">
                        ...
                        <a href="{{ route('admin/modules/edit', $rs->id)}}" class="text-green-800 pl-2">
                        ...
                        <form action="{{ route('admin/modules/destroy', $rs->id) }}" method="POST" onsubmit="return confirm('Delete?')" class="float-right text-red-800">
                            @csrf
                            @method('DELETE')
                            <button>
